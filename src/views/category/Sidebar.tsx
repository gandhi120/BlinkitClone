...
useEffect(()=>{
    ...
    categories?.forEach((category:any,index:number)=>{
        ...
        animatedValues[index].value = withTiming(isSelected ? 0 : -10,{duration:500});
          ...
          <View>
            {categories?.map((category:any,index:number)=>{
              const animatedStyle = useAnimatedStyle(()=>{
                const bottom = animatedValues[index].value;
                return{bottom};
              });
              return(
                <TouchableOpacity
                ...
                style={styles.categoryButton}
                 onPress={()=>onCategoryPress(category)}>
                      ...
                      <CustomText fontFamily={Fonts.SemiBold}  fontSize={RFValue(7)} style={{textAlign:'center'}}>
                        {category?.name}
                      </CustomText>
